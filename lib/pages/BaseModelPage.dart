...
class BaseModelPage extends BasePage {
  ...
  static Future<void> showSortPopup(StateData stateData) {
    return showDialog(
        ...
        builder: (_) {
          return SortDialog(
            ornekKayit: stateData.tag["ornekKayit"],
            orderBy: stateData.tag["orderBy"],
            baseModelPageId: stateData.pageId,
          );
        });
  }
  ...
  static Future<void> showFilterPopup(StateData stateData) {
    return showDialog(
        ...
        builder: (_) {
          return FilterDialog(
            ornekKayit: stateData.tag["ornekKayit"],
            filterMap: stateData.tag["filterMap"],
            baseModelPageId: stateData.pageId,
            scaffoldKey: stateData.scaffoldKey,
          );
        });
  }
  ...
  static Future<void> refreshData(StateData stateData, {int? currentPagePrm, String? orderByPrm, Map<String, dynamic>? filterMapPrm}) async {
    ...
    var map = await BaseModel.getList(stateData.tag["ornekKayit"],
        rawQuery: stateData.tag["getListQuery"],
        pageSize: stateData.tag["pageSize"],
  ...
  BaseModelPage({
    ...
    this.topActions,
            ...
            pageTitle: pageTitle ??
                (stateData) {
                  return BaseModel.createNewObject(modelName).pageTitle;
                },
            initialTag: (_) => {
                    ...
                    child: Row(
                        children: []
                          ..addAll(addButtonTitle != null
                              ? [
                                  Expanded(
                                    child: SntIconButton(
                                      ...
                                      icon: Icons.add,
                                      onTap: () async {
                                        await NavigatorBase.push(BaseModelDuzenleme(
                                          widgetKayit: stateData.tag["selectedKayit"],
                                          widgetModelName: modelName,
                                          baseModelPageId: stateData.pageId,
                                          baseModelPageScaffoldKey: stateData.scaffoldKey,
                                        ));
                                        return;
                                      },
                                    ),
                              ...
                              : [])
                          ..addAll(topActions == null
                              ? []
                              : topActions(stateData).map((action) {
                                  return Expanded(
                                    child: SntIconButton(
                          ...
                          ..addAll([
                            Expanded(
                              child: SntIconButton(
                                ...
                                color: stateData.tag["filterMap"] != null && stateData.tag["filterMap"].length > 0
                                    ? ConstantsBase.greenAccentShade100Color
                                    : ConstantsBase.defaultButtonColor,
                            ...
                            Expanded(
                              child: SntIconButton(
                                ...
                                color: stateData.tag["orderBy"] != null && stateData.tag["orderBy"].isNotEmpty
                                    ? ConstantsBase.greenAccentShade100Color
                                    : ConstantsBase.defaultButtonColor,
            ...
            body: (StateData stateData) {
              return Container(
                ...
                child: stateData.tag["currentListData"] == null
                    ? Center(child: CircularProgressIndicator())
                    : ListView(
                        children: stateData.tag["currentListData"]
                            .asMap()
                            .entries
                            .map<Widget>((entry) => Container(
                                  decoration: BoxDecoration(
                                    //border: Border.all(width: 1),
                                    color: stateData.tag["selectedKayit"] != null && stateData.tag["selectedKayit"].get("ID") == entry.value.get("ID")
                                        ? Colors.yellow
                                        : (entry.value.listBgColor != null ? entry.value.listBgColor(entry.value) : Colors.white),
                                  ),
                                  child: Slidable(
                                      //startActionPane: ActionPane(motion: const DrawerMotion(), extentRatio: 0.25, children: []),
                                      child: Card(
                                          color: entry.key % 2 == 0 ? Colors.white : ConstantsBase.greenAccentShade100Color,
                                          elevation: 5.0,
                                          child: ListTile(
                                            selected:
                                                stateData.tag["selectedKayit"] != null && stateData.tag["selectedKayit"].get("ID") == entry.value.get("ID"),
                                            onLongPress: () {
                                          ...
                                          ..addAll(editButtonTitle != null
                                              ? [
                                                  SlidableAction(
                                                    ...
                                                    icon: Icons.edit,
                                                    onPressed: (context) {
                                                      NavigatorBase.push(BaseModelDuzenleme(
                                                          widgetKayit: entry.value,
                                                          widgetModelName: modelName,
                                                          baseModelPageId: stateData.pageId,
                                                          baseModelPageScaffoldKey: stateData.scaffoldKey));
                                                    },
              ...
              return Row(
                ...
                children: <Widget>[
                  ...
                  Expanded(
                      ...
                      child: Center(
                        child: Text(
                          stateData.tag["totalCount"].toString() +
                              " " +
                              ConstantsBase.translate("kayit") +
                              " - " +
                              stateData.tag["currentPage"].toString() +
                              " / " +
                              stateData.tag["lastPage"].toString(),
                  ...
                  Expanded(
                    ...
                    child: SntIconButton(
                        color:
                            stateData.tag["currentPage"] >= stateData.tag["lastPage"] ? ConstantsBase.defaultDisabledColor : ConstantsBase.defaultButtonColor,
